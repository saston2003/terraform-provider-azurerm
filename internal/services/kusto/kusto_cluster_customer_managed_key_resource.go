...
func resourceKustoClusterCustomerManagedKeyCreateUpdate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if cluster.Model == nil || cluster.Model.Properties == nil {
		return fmt.Errorf("retrieving %s: `Properties` was nil", clusterID)
	}
	...
	if err := clusterClient.UpdateThenPoll(ctx, *clusterID, props, clusters.UpdateOperationOptions{IfMatch: utils.String("")}); err != nil {
...
func resourceKustoClusterCustomerManagedKeyRead(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if err != nil {
		if response.WasNotFound(cluster.HttpResponse) {
			log.Printf("[DEBUG] %s could not be found - removing from state!", clusterID)
			d.SetId("")
			return nil
		}
	...
	if cluster.Model.Properties.KeyVaultProperties == nil {
		log.Printf("[DEBUG] Customer Managed Key was not defined for %s - removing from state!", clusterID)
		d.SetId("")
		return nil
	}
	...
	if keyVaultURI == "" {
		return fmt.Errorf("retrieving %s: `properties.keyVaultProperties.keyVaultUri` was nil", clusterID)
	}
...
func resourceKustoClusterCustomerManagedKeyDelete(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if err = client.UpdateThenPoll(ctx, *clusterID, props, clusters.UpdateOperationOptions{IfMatch: utils.String("")}); err != nil {
